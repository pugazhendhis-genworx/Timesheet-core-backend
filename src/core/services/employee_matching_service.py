...
async def match_employees_for_timesheet(
    ...
    """
    logger.info(f"Starting employee matching for timesheet_id={timesheet_id}")
    try:
        ...
        logger.info(
            f"Matching {len(entries)} entries against {len(all_employees)} employees "
            f"for timesheet_id={timesheet_id}"
        )
        ...
        for idx, entry in enumerate(entries):
            ...
            logger.debug(
                f"Processing entry_id={entry.timeentry_id}, "
            ...
            if matched_employee:
                ...
                await db.flush()
                logger.debug(
                    f"""Entry {entry.timeentry_id} matched
                    to employee_id={matched_employee.employee_id}"""
                )
                # Validate assignment
        ...
        logger.error(
            f"Fatal error in employee matching for timesheet_id={timesheet_id}",
            exc_info=True,
        )
        raise
...
async def validate_client_for_timesheet(
    ...
    """
    logger.info(f"Starting client validation for timesheet_id={timesheet_id}")
