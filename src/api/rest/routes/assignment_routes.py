...
async def get_assignments(db=Depends(get_pg_session)):
    return await get_all_assignments_service(db)


@assignment_router.get("/{assignment_id}", response_model=AssignmentResponse)
async def get_assignment(assignment_id: UUID, db=Depends(get_pg_session)):
...
async def get_assignments_by_employee(employee_id: UUID, db=Depends(get_pg_session)):
...
async def get_assignments_by_client(client_id: UUID, db=Depends(get_pg_session)):
...
async def assign_employee(
    assignment_data: AssignmentCreate, db=Depends(get_pg_session)
):
...
async def delete_assignment(assignment_id: UUID, db=Depends(get_pg_session)):
